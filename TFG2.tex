...
\end{defin}
Sea $M$ una variedad, $p\in M$. Si $U\subseteq M$ es un entorno de $p$ que es homeomorfo a un abierto $U'$ de $\R^n$, decimos que $U$ es un \enfatiza{dominio coordenado}, llamamos \enfatiza{aplicación coordenada} a cualquier homeomorfismo $\varphi :U\to U'$ y decimos que el par $(U,\varphi )$ es una \enfatiza{carta para $M$}.
...
Sea $M$ una variedad con borde, entonces $\interior{M}$ es un abierto de M, y es en si mismo una $n$-variedad.
\end{prop}
\begin{proof}
...
Veamos algunos ejemplos no triviales de variedades con borde:
\begin{eje}
\begin{itemize}
\item[(1)] El cilindro $Cil=\left\{(x,y,z)\in \R^3 \mid x^2+y^2=1,\, z\in \left[ 0,1\right] \right\}$.
\item[(2)] La banda de Möbius\\
...
En esta sección daremos una demostración clásica del teorema de clasificación de superficies compactas, que fue probado por primera vez en 1907 por Max Dehn y Poul Heegaard \cite{dehn}. Me baso en la demostración de Lee \cite{lee1}, que a su vez se basa en la de Seifert y Threlfall \cite{seifert}.
...
Como adelantamos en \autoref{sec:variedades}, para el teorema de clasificación necesitamos un método uniforme de representación de las superficies compactas. Representaremos todas las superficies como cocientes de regiones poligonales con $2n$ lados. 
...
\end{tikzpicture}
\caption{\autoref{eje:rep_perf} \label{fig:rep_perf}}
\end{figure}
...
\bibitem{seifert}
H. Seifert, W. Threlfall. \textit{A Textbook of Topology}, 1era edición. Academic, New York, 1980.
\end{thebibliography}
