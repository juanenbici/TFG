...
Si $M$ es una superficie con borde, entonces un punto $p\in M$ no puede ser un punto interior y un punto borde al mismo tiempo. O lo que es lo mismo, $M=\interior{M} \cup \partial M$, con $\interior{M} \cap \partial M=\emptyset$.
\end{prop}
\begin{proof}
Supongamos que $p\in M$ es tanto un punto interior como un punto borde de $M$. Entonces existen $(U,\varphi)$ tal que $\varphi (U)\subseteq \interior{\mathbb{H}^2}$ y $(V,\psi)$ tal que $\psi (V)\subseteq \mathbb{H}^2$, donde $\psi (p)=s\in \partial \mathbb{H}^2$. 
\end{proof}
La propiedad anterior se puede generalizar para una $n$-variedad con borde, aunque no procede demostrarlo. -¿Cómo ponerlo mejor?-
\begin{corol}
