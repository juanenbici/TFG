...
Por otro lado, vamos a ver que \textit{pegando} discos a las componentes de $\partial M$ se obtiene una superficie sin borde. Sean $S=\amalg_i \overline{\mathbb{B}}_i^2$ y $f:\partial S=\amalg_i \mathbb{S}^1\to \partial M$ un homeomorfismo. Considerando $f$ como una función de $\partial S$ en $M$, definimos la relación de equivalencia $\sim$ en la unión disjunta $M\amalg \S$ generada por $a\sim f(a)$, y denotamos el espacio cociente resultante por $X=(M\amalg S)/\sim$. Tenemos que ver que $X$ es IIAN, Haussdorf y localmente Euclídeo de dimensión $n$. Empecemos con esto último. 

Sea $\q:M\amalg S\to X$ la aplicación cociente, y sea $B=q(\partial M\cup \partial S)$. Notamos que $\interior M\amalg \interior S$ es un abierto saturado de $M\amalg S$, por lo que podemos restringir $q$ a una aplicación cociente que va de $\interior M\amalg \interior S$ en $X\setminus B$. Dado que esta restricción es inyectiva, es un homeomorfismo, y por tanto $X\setminus B$ es localmente Euclídeo de dimensión $n$. Por tanto nos queda considerar los puntos en $B$, pero esto se obtiene por el mismo argumento que usamos en la demostración de \autoref{prop:poligonos}. 

Por otro lado, el espacio cociente $X$ es IIAN por la proposición \autoref{prop:sub_IIAN}. Para probar que es Haussdorf, hay que ver que las fibras de $q$ se pueden separar por abiertos saturados. 
...
\begin{eje}
Sea $M$ una superficie. Si retiramos una serie de discos abiertos disjuntos (una definición precisa de este procedimiento se dará en \autoref{def:perforacion}), entonces obtenemos una superficie con borde.
\end{eje}
