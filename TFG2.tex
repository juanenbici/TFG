...
B\circ \psi (y), & y\in V.\\
\end{cases}$$
Dado que $U\amalg V$ es un abierto saturado de $M$, la restricción $\pi|_{U\amalg V}$ es una aplicación cociente en el entorno de $d$ $\pi (U\amalg V)$, y por \autoref{ec:1} $\tilde{\Phi}$ pasa al cociente y define una aplicación continua e inyectiva $\Phi :q(U\amalg V)\to \R^2$. Como $\varphi$, $\psi$ y $B$ son homeomorfismos entre sus dominios y sus imágenes, podemos definir la inversa de $\Phi$ de la siguiente manera:
$$\Phi ^{-1}(s)= \begin{cases} 
