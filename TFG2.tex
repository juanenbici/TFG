...
\begin{proof}
Sea $M$ el espacio cociente, sea $\pi :P\to M$ la aplicación cociente y sean $M_0, \, M_1, \, M_2=M$ respectivamente las imágenes por $\pi$ de los vértices, los bordes y las regiones poligonales. Por la propia definición de $P$, $M_0$ es un espacio discreto, y para $K=1,2$, $M_k$ se obtiene a partir de $M_{k-1}$ pegando un número finito de $k$-celdas. Por tanto $M$ es un CW-complejo por la \autoref{def:cw}.\\
...
Un punto $s$ de una 1-celda tiene exáctamente dos preimágenes $x_1$ y $x_2$, cada una en el interior de un borde distinto $A_i, \, i=1,2$. Supongamos sin pérdida de generalidad que estos dos bordes pertenecen respectivamente a $P_1$ y $P_2$ ($P_1$ puede ser igual a $P_2$), y denotemos por $h:A_2\to A_1$ es un homeomorfismo. Dado que cada $P_i$ es una variedad con borde, y $x_1,\, x_2$ son puntos borde, podemos elegir cartas coordenadas $(U_i,\varphi_i )$ para $P_i$ tal que $x_i\in U_i$. Denotamos $\widehat{U}_i=\varphi_i(U_i)\subseteq \mathbb{H}^2, \, i=1,2$ y podemos asumir, contrayendo los $U_i$ si es necesario, que $h(U_2\cap A_2)=U_1\cap A_1$, y que $\widehat{U}_i=U_i'\times [0,\varepsilon)$ para un $\varepsilon >0$ y unos subconjuntos $U_i'\subset \R$ como se muestra en la \autoref{fig:6_4}. Así pues podemos escribir las aplicaciones coordenadas como $\varphi_i(x_i)=(\varphi_{(i,0)}(x_i),\varphi_{(i,1)}(x_i))$, con $\varphi_{(i,0)}:U_i\to U_i'$ y $\varphi_{(i,1)}:U_i\to [0,\varepsilon)$ aplicaciones continuas. Que cada $x_i$ sea un punto borde significa que $\varphi_{(i,1)}(x_i)=0$.\\
Queremos ensamblar estas dos cartas con una aplicación cuya imagen sea un abierto de $\R^2$, pegándolas por los puntos que se corresponden en $A_1$ y $A_2$. El problema es que las aplicaciones $\varphi_i$ no tienen por qué llevar puntos borde que se corresponden al mismo punto imagen, y por tanto tenemos que ajustarlo. Se tiene que las dos restricciones $\varphi_{(i,0)}|_{U_i\cap A_i}: U_i\cap A_i\to U_{i}'$ son homeomorfismos, y definimos así el homeomorfismo $\beta:U_2'\to U_1'$ por $$\beta=(\varphi_{(1,0)}|_{U_1\cap A_1})\circ h\circ (\varphi_{(2,0)}|_{U_2\cap A_2})^{-1}\, .$$ 
Sea ahora $B:\widehat{U}_2\to \R^2$ la aplicación $$B(x_{(2,1)},x_{(2,2)})=\left(\beta (x_{(2,1)}), -x_{(2,2)})\, .$$
Geométricamente, $B$ junta los puntos borde de acuerdo con $\beta$, y ``da la vuelta'' a cada segmento vertical por encima de el borde a un segmento debajo del punto imagen (como en \autoref{fig:6_4}). 
\section{!!!!!!}
\end{proof}
