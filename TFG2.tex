...
\begin{corol}
Si $M$ es una $n$-variedad con borde, entonces $\partial M$ es un cerrado en $M$\begin{comment}, $\partial M$ es una $(n-1)$-variedad\end{comment}
. Por otro lado, $M$ es una variedad si y solo si $\partial M=\emptyset$.
\end{corol}

\begin{proof}
Por el teorema de la invarianza del borde, $\partial M= M\setminus \interior{M}$, y siendo $\interior{M}$ abierto, $\partial M$ es por tanto cerrado. Si $M$ es una variedad, entonces todo punto está en el dominio de una carta interior, por lo que $M=\interior{M}$, y se sigue de la invarianza del borde que $\partial M=\emptyset$. Si suponemos ahora que $M=\partial M$, entonces $M=\interior{M}$, que es una variedad.
\end{proof}
