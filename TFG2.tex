...
\begin{prop}
Sea $X$ un espacio topológico. Supongamos que $U,V\subseteq X$ son abiertos de $X$ tales que $X=U\cup V$. Supongamos también que $U$, $V$ y $U\cap V$ son conexos por caminos. Sea $p\in U\cup V$, y supongamos también que $U$ es simplemente conexo. Entonces la inclusión $l: V\xhookrightarrow{} X$ induce el isomorfismo
\[
...
\end{tma}
\begin{proof}
Denotemos por $\mathcal{P}$ la representación de $M$ dada, y sea $\pi$ la aplicación cociente $\pi: |\mathcal{P}| \to M$. Sea $M_1$ la imagen por $\pi$ del conjunto de las aristas. Se tiene que $M_1$ es una suma wedge de circunferencias $\mathbb{S}^1$, una por cada símbolo de la representación. Así pues, su grupo fundamental tiene como presentación $\langle a_1, \dots ,a_n \mid \emptyset \rangle$. Si ahora aplicamos el \autoref{teo:2celda} (teniendo en cuenta que la región poligonal es homeomorfa al disco cerrado unidad), tenemos que la aplicación $\varphi$ del teorema envía la frontera del polígono al lazo en $M_1$ obtenido siguiendo los generadores en el orden especificado por  la palabra $W$, obteniendo así el resultado.
\end{proof}
