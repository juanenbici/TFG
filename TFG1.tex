...
\fill [color=black] (5,-2) circle (1.5pt);
\end{scriptsize}
\draw (-7.8,0) node[anchor=west] {$ a $};
\draw (-2.2,0) node[anchor=east] {$ a $};
\draw (2.2,0) node[anchor=west] {$ a $};
\draw (7.8,0) node[anchor=east] {$ a $};
...
Un hecho fundamental para la prueba del teorema de clasificación es que toda superficie es triangulable. Este teorema, probado por Radó, es largo y técnico de probar. Las demostraciones clásicas utilizan el \emph{teorema de Schönflies}, el cual no es fácil de probar. Intentaremos aquí dar una aproximación con el truco de Kirby para superficies \cite{hatcher_torus}.
