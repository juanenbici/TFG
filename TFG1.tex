...
%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%
Para el teorema de clasificación necesitamos un método uniforme de representación de las superficies compactas. Trataremos de dar una forma de representarlas como CW complejos. Veremos que toda superficie compacta se puede representar en el plano como un cociente de una región poligonal por una relación de equivalencia que identifica sus lados dos a dos. Empecemos viendo tres figuras elementales: la esfera $\Esfera$, el plano proyectivo $\Proyectivo$ y el toro $\Toro$. Como veremos, estos ejemplos son fundamentales pues toda superficie compacta se puede construir a partir de ellas.
...
\caption{La esfera como cociente del disco $\overline{\mathbb{B}}^2$.}
\end{figure}


\begin{prop}%%%%PROP: ESFERA COCIENTE DISCO Y CUADRADO%%%%
\label{teo:Esfera como cociente de disco y cuadrado}
...
\begin{figure}[h]%%%%FIGURA: ESFERA CUADRADO
\begin{center}
...
\fill [color=uququq] (0,2) circle (1.5pt);
\fill [color=uququq] (5.34,-0.66) circle (1.5pt);
\end{scriptsize}
\end{tikzpicture}
...
\label{teo:toro_cuadrado}
El toro $\Toro$ es homeomorfo al espacio cociente resultante de la relación de equivalencia en el cuadrado $I\times I$ que identifica $(x,0)\sim (x,1)$ para todo $x\in I$, y $(0,y)\sim(1,y)$ para todo $y\in I$ (\ref{fig:toro_cuadrado}. 
\end{prop}
\begin{proof}
Definimos la aplicación $q:I\times I \to \Toro$ que manda $q(u,v)=(e^{2\pi iu},e^{2\pi iv})$. Por el teorema de la aplicación cerrada (\ref{teo:aplicac_cerrada}), es una aplicación cociente. Al hacer las mismas identificaciones que la relación de equivalencia, por la unicidad del espacio cociente (Teorema \ref{teo:unicidad_espacio_cociente}) se obtiene el resultado.
\end{proof}


\begin{figure}%%%%FIGURA: TORO COMO CUADRADO
\label{fig:toro_cuadrado}
\begin{center}
\definecolor{uququq}{rgb}{0.25,0.25,0.25}
\definecolor{tttttt}{rgb}{0.2,0.2,0.2}
\begin{tikzpicture}[use optics][line cap=round,line join=round,>=triangle 45,x=2.0cm,y=2.0cm]
%\clip(-4.46,-3.51) rectangle (4.94,2.85);
\draw [->-={at=0.5}](-8,-2)-- (-8,2);
\draw [->>-={at=0.5}](-8,2)-- (-4,2);
\draw [-<-={at=0.5}](-4,2)-- (-4,-2);
\draw [->>-={at=0.5}](-8,-2)-- (-4,-2);
...
\begin{scriptsize}
\fill [color=tttttt] (-8,-2) circle (1.5pt);
\fill [color=tttttt] (-8,2) circle (1.5pt);
\fill [color=tttttt] (-4,2) circle (1.5pt);
\fill [color=tttttt] (-4,-2) circle (1.5pt);
\fill [color=uququq] (-2.5,0) circle (1.5pt);
\fill [color=uququq] (1.5,0) circle (1.5pt);
\fill [color=uququq] (5.8,-0.9) circle (1.5pt);
\end{scriptsize}
\end{tikzpicture}
\end{center}
\caption{El toro como cociente de un cuadrado.}
\end{figure}


\begin{prop}%%%%PROP: PLANO PROYECTIVO COMO CUADRADO
\label{teo:proyectivo_cociente_cuadrado}
...
Si $F:\overline{\mathbb{B}}^2 \to \mathbb{S}^2$ es la aplicación que manda el disco al emisferio superior de la esfera por $F(x,y)=(x,y,\sqrt{1-x^2-y^2})$, entonces $p\circ F:\overline{\mathbb{B}}^2 \to \mathbb{S}^2/\sim$ es sobreyectiva (---lo demuestro?) y es así una aplicación cociente por el teorema de la aplicación cerrada (\ref{teo:aplicac_cerrada}). La aplicación identifica únicamente $(x,y)\in \partial \overline{\mathbb{B}}^2$ con $(-x,-y)\in \partial \overline{\mathbb{B}}^2$, por lo que $\mathbb{P}^2$ es homeomorfo al espacio cociente resultante.
 Para la parte (b) hacemos como en la demostración de la Proposición \ref{teo:Esfera como cociente de disco y cuadrado} (b). 
\end{proof}
\begin{figure}[h]%%%%FIGURA: PLANO PROYECTIVO COMO CUADRADO
\label{fig:plano_proyectivo_cuadrado}
\begin{center}
\definecolor{uququq}{rgb}{0.25,0.25,0.25}
\begin{tikzpicture}[use optics][line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
...
\end{scriptsize}
\end{tikzpicture}
\caption{Representación de $\mathbb{P}^2$ como un espacio cociente.}
\end{center}
\end{figure}
...
\end{tikzpicture}
\caption{Suma conexa de un toro y una esfera.}
\end{figure}
...
\begin{obs}%%%%OBS: Suma conexa variedad y esfera
Si $V$ es una $n$-variedad, la suma conexa $V\#\Esfera$ es homeomorfa a $V$, al menos si hacemos las elecciones correctas (Fig. \ref{fig:suma_toro_esfera})
\end{obs}
...
%\draw plot [smooth cycle, tension=0.1] coordinates {(-4.5,0) (-3.914213562,1.414213562) (-2.5,2) (-1.5,1.732050808) (0,1)(1.5,1.732050808) (2.5,2) ((3.914213562,1.414213562) (4.5,0) (3.914213562,-1.414213562) (2.5,-2) (1.5,-1.732050808) (0,-1) (-1.5,-1.732050808) (-2.5,-2) (-3.914213562,-1.414213562)};

\draw (-1.5,1.732050808) arc[x radius=2, y radius=2, start angle=60, end angle=300];
\draw (1.5,-1.732050808) arc[x radius=2, y radius=2, start angle=240, end angle=480];
\draw plot [smooth] coordinates { (-1.5,1.732050808) (0,1)(1.5,1.732050808)};
\draw plot [smooth] coordinates {(1.5,-1.732050808) (0,-1) (-1.5,-1.732050808)};

\draw (-1.8,0.35) arc[x radius=0.7, y radius=0.6, start angle=0, end angle=-180];
\draw (-2,0) arc[x radius=0.5, y radius=0.5, start angle=0, end angle=180]; 
\draw (3.2,0.35) arc[x radius=0.7, y radius=0.6, start angle=0, end angle=-180];
\draw (3,0) arc[x radius=0.5, y radius=0.5, start angle=0, end angle=180]; 
...
Como ya hemos dicho anteriormente, para el teorema de clasificación necesitamos un método uniforme de representación de las superficies compactas. Representaremos todas las superficies como cocientes de polígonos con $2n$ lados. 
...
Se tiene que 
\begin{itemize}
\item[a)] $\mathbb{S}^2=\langle a|aa^{-1}\rangle=\langle a,b| abb^{-1}a^{-1}\rangle$ (Proposición \ref{teo:Esfera como cociente de disco y cuadrado})
\item[b)] $\mathbb{P}^2= \langle a|aa \rangle = \langle a,b|abab \rangle$ (Proposición \ref{teo:proyectivo_cociente_cuadrado})
\item[c)] $\Toro=\langle a,b|aba^{-1}b^{-1}\rangle$ (Proposición \ref{teo:toro_cuadrado})
\end{itemize}
\end{eje}


\begin{figure}%%%%FIG: Representación de superficies impoirtantes
\label{fig:Representacion_superficies_importantes}
\centering
\begin{tikzpicture}[use optics][line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\draw [->-](0,2)-- (2,0);
\draw [->-](2,0)-- (0,-2);
...
\draw (-4,1.5) node[anchor=north west] {$ a $};
\draw (-4,-1) node[anchor=north west] {$ b $};
\draw (-6,1.5) node[anchor=north east] {$ a $};
\draw (-6,-1) node[anchor=north east] {$ b $};

\draw (1,1.5) node[anchor=north west] {$ a $};
\draw (1,-1) node[anchor=north west] {$ b $};
\draw (-1,1.5) node[anchor=north east] {$ b $};
\draw (-1,-1) node[anchor=north east] {$ a $};

\draw (6,1.5) node[anchor=north west] {$ a $};
\draw (6,-1) node[anchor=north west] {$ b $};
\draw (4,1.5) node[anchor=north east] {$ b $};
\draw (4,-1) node[anchor=north east] {$ a $};

\draw (5.1,-2.5) node[anchor=south] {$ \Toro $};
\draw (0.1,-2.5) node[anchor=south] {$ \Proyectivo $};
\draw (-4.9,-2.5) node[anchor=south] {$ \Esfera $};
...
\begin{comment}%%%%REPRESENTACIONES DE S1 Y P2....SON UNA MIERDA MEJOR REPETIRLAS
\begin{figure}
