...
\centering
\begin{tikzpicture}[use optics, line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, scale=0.9]
\draw [->-={at=0}, -<-={at=0.5}](-3,0) circle (2cm);
\draw [->-={at=0}, ->-={at=0.5}](3,0) circle (2cm);
\begin{scriptsize}
\fill [color=black] (-3,2) circle (1.5pt);
\fill [color=black] (-3,-2) circle (1.5pt);
...
\fill [color=black] (3,-2) circle (1.5pt);
\end{scriptsize}
\draw (-5.8,0) node[anchor=west] {$ a $};
\draw (-0.2,0) node[anchor=east] {$ a $};
\draw (0.2,0) node[anchor=west] {$ a $};
\draw (5.8,0) node[anchor=east] {$ a $};
\draw (-2.9,-2.9) node[anchor=south] {$ \Esfera $};
\draw (3.1,-2.9) node[anchor=south] {$ \Proyectivo $};
\end{tikzpicture}
